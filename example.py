...
class Ghost:
    ...
    def check_collisions(self):
        ...
        if 0 < self.center_x // 30 < 29:
            ...
            if self.direction == 2 or self.direction == 3:
                if center_left <= self.center_x % num2 <= center_right:
                    if level[(self.center_y + num3) // num1][self.center_x // num2] < 3 \
                            or (level[(self.center_y + num3) // num1][self.center_x // num2] == 9 and (
                            self.in_box or self.dead)):
                        self.turns[3] = True
                    if level[(self.center_y - num3) // num1][self.center_x // num2] < 3 \
                            ...
                            self.in_box or self.dead)):
                        self.turns[2] = True
                if center_left <= self.center_y % num1 <= center_right:
    ...
    def move_blinky(self):
        ...
        num3 = 15
        # self.x_pos += self.speed
        currentPositionGhost = (((self.center_y - num3) // num1), (self.center_x // num2))
        currentPositionPacman = (((player_y + 25 - num3) // num1),((player_x + 23) // num2))
        print('current position: ', currentPositionGhost)
        print('pacman postion: ', currentPositionPacman)
        listDiretion = a_star(level, currentPositionGhost, currentPositionPacman)
        print('diretion list: ', listDiretion)

        if len(listDiretion) == 0:
        ...
        if listDiretion[0] == 0 and self.turns[0]:
            self.direction = 0
            self.x_pos += self.speed
        elif listDiretion[0] == 1 and self.turns[1]:
            self.direction = 1
            self.x_pos -= self.speed
        elif listDiretion[0] == 2 and self.turns[2]:
            self.direction = 2
            self.y_pos -= self.speed
        elif listDiretion[0] == 3 and self.turns[3]:
            self.direction = 3
            self.y_pos += self.speed
...
def a_star(grid, start, target):
    ...
    def get_neighbors(node):
        """Returns valid neighbors with direction."""
        directions = [
            ((0, 1), 0),  # right
            ((0, -1), 1),  # left
            ((-1, 0), 2),  # up
        ...
        for (dx, dy), direction in directions:
            ...
            if  0 <= x < rows and 0 <= y < cols and grid[x][y] < 3:
                yield (x, y), direction

    open_list = []
    heapq.heappush(open_list, (0, start))  # (F-cost, node)
    came_from = {}  # {current_node: (parent_node, direction)}
    g_cost = {start: 0}
    ...
    while open_list:
        ...
        if current == target:  # Path found
            ...
            return directions[::-1]  # Reverse the direction list
...
def check_position(centerx, centery):
    ...
    if centerx // 30 < 29:
        ...
        #between (center_left, center_right), we can turn easily
        center_left = 8
        center_right = 20
        if direction == 2 or direction == 3:
            if center_left <= centerx % num2 <= center_right:
                if level[(centery + num3) // num1][centerx // num2] < 3:
                    turns[3] = True
                if level[(centery - num3) // num1][centerx // num2] < 3:
                    turns[2] = True
            if center_left <= centery % num1 <= center_right:
